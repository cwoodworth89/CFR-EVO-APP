...
1. **Docker Compose Missing Tile & OSRM Services**:
   ...
   - The `api` service was missing `OSRM_BACKEND_URL: http://osrm:5000`, `TILE_SERVER_URL: http://tiles:8080`, and healthcheck conditions for database, broker, routing, and tile container dependencies.
...
4. **Premise 4 (Automatic Error Fallback)**: By implementing `FallbackTileLayer` (extending `L.TileLayer` in `MapLayers.jsx` with event listeners on `load` and `error`), when a local tile is not found or the server is starting up, Leaflet intercepts the error and transparently retries against `fallbackUrl`.
5. **Premise 5 (Centralized Kiosk UI Basemap Rendering)**: Migrating `RouteOverviewPanel.jsx` and `BlockParcelPanel.jsx` to consume `<BaseMap style="VOYAGER" />` and `<BaseMap style="GREY" />` standardizes map tile resolution, error handling, and `maxNativeZoom: 18` / `maxZoom: 22` overscaling across the entire kiosk HUD.
...
1. **Docker Compose**: Added `cfr_tiles` (`consbio/mbtileserver` on `8081:8080`) and `cfr_osrm` (`osrm/osrm-backend:v5.27.1` on `5000:5000`) with comprehensive health checks (`wget`, `curl`, `pg_isready`, `mosquitto_sub`). Added `OSRM_BACKEND_URL: http://osrm:5000` and `TILE_SERVER_URL: http://tiles:8080` to `cfr_api`.
2. **API & Dynamic Resolution**: Exported `TILE_BASE_URL`, `getTileUrl()`, and `getTileLayerConfig()` in `frontend/src/apiClient.js` with `maxNativeZoom: 18` and `maxZoom: 22`.
3. **Leaflet Basemaps**: Updated `MapConstants.js` and `MapLayers.jsx` with local tile endpoints, `maxNativeZoom: 18`, and dynamic online fallback handling via `FallbackTileLayer`.
4. **Kiosk Panels**: Verified and updated `RouteOverviewPanel.jsx` and `BlockParcelPanel.jsx` to consume `BaseMap` (`VOYAGER` and `GREY`) without hardcoded external URLs.
5. **Build & Test Verification**: `npm.cmd run build` completed with zero errors in 2.60s; all 20 backend routing tests passed with 100% success rate.
