...
* **118 Active Emergency Response Zones (Zones 1–134)**: The City's emergency response zone polygons in `public.zones`, from the City's Planning map service. They give every lot its map grid, taken at the lot's centre point.
...
* **Centralized MBTiles Server (`cfr_tiles` on Port 8081)**: Fast, offline tile serving powered by `ghcr.io/consbio/mbtileserver:latest`. Strictly adheres to the **OpenStreetMap Slippy Map Specification** (XYZ Web Mercator `EPSG:3857`, top-left origin) across all base layers and overlays (the `ortho` aerial imagery, the `street_vector` OpenStreetMap vector basemap, and the transparent municipal `cadastral` parcel/address overlay).
