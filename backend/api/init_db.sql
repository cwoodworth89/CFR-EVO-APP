...
-- Indices for dispatches performance
CREATE INDEX IF NOT EXISTS idx_dispatches_timestamp ON public.dispatches (timestamp DESC);
CREATE INDEX IF NOT EXISTS idx_dispatches_target_gin ON public.dispatches USING gin (target jsonb_path_ops);
CREATE INDEX IF NOT EXISTS idx_dispatches_routing_metrics_gin ON public.dispatches USING gin (routing_metrics jsonb_path_ops);
CREATE INDEX IF NOT EXISTS idx_dispatches_feedback_verified ON public.dispatches (timestamp DESC) WHERE feedback_submitted = TRUE AND verified_address IS NOT NULL;
