-- CFR EVO apparatus profile -- rules only.
--
-- Derived 2026-09-09 from the stock car.lua shipped in the OSRM image that built the kiosk
-- graph (osrm-backend v26.8.0; car.lua md5 db4f09652994b1a24389de0f0a6b7719, vendored with its
-- lib/ at docs/standards/osrm/). The complete change set is
--     diff docs/standards/osrm/car.lua backend/osrm/profiles/apparatus.lua
-- and every hunk below cites the operator ruling it implements, recorded 2026-09-09 in
-- docs/punchlist/01-erratic-routing-loops-intra-municipal-path-preference.md.
--
-- What changes: turn-restriction relations are not applied, and fire lanes are drivable.
-- What does not: the speed table, the turn and U-turn costs, one-way handling, barriers,
-- access tags, and the car's dimension checks. The speeds are OSRM's own and unmeasured for
-- this city or these vehicles (CLAUDE.md section 6.3); they move only against measured
-- travel times, and never here by hand.
--
-- Needs the image's lib/ beside it: mount this file as /opt/apparatus.lua, which
-- backend/scripts/build_osrm_graph.sh does.
...
function setup()
  return {
    properties = {
      ...
      -- Operator ruling 2026-09-09: "Ignore turn restrictions, we make those turns under
      -- lights and siren." car.lua line 29 has true. Every restriction relation in the
      -- extract is left out of the graph; one-way, barriers and access tags are unaffected,
      -- and u_turn_penalty / turn_penalty below still price the turn.
    ...
    -- Operator ruling 2026-09-09: "Fire lanes a car can definitely use, but not park."
    -- car.lua lines 137-139 forbid service=emergency_access outright. Nothing is forbidden
    -- by service tag here; apparatus_access below handles the access tag on a fire lane.
...
-- Operator ruling 2026-09-09: "Fire lanes a car can definitely use, but not park."
-- A fire lane in OSM is highway=service + service=emergency_access. Of the City's 13, six also
-- carry access=no and two access=private (extract read 2026-09-09): the mapper saying who the
-- public is. For an apparatus the lane exists to be driven, so the access tag is not consulted
-- on it; every other way goes through the shipped WayHandlers.access unchanged.
--
-- 'emergency' and 'emergency_vehicle' stay in access_tag_blacklist on purpose. The only City
-- ways carrying those values are nine footways and flights of steps, which are not roads, and
-- removing the values would route them at default_speed through WayHandlers.speed's fallback
-- (lib/way_handlers.lua line 308).
local function apparatus_access(profile, way, result, data)
  if way:get_value_by_key('service') == 'emergency_access' then
