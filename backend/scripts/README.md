...
| Script | Purpose |
|:--|:--|
| `export_routing_polygon.py` | Writes `public.city_boundary` buffered 100 m as one GeoJSON Feature for `osrm-extract --location-dependent-data`, the polygon the apparatus profile uses to keep routes in the City (operator ruling 2026-09-09: boundary + 100 m). Regenerated by `build_osrm_graph.sh ... city` at every build so it cannot drift from its source; never committed. |
...
Repairs to records already written. Each is a dry run by default.

| Script | Purpose |
|:--|:--|
| `backfill_routing_metrics.py` | Recomputes `routing_metrics` for dispatches whose stored ETAs never followed their coordinates -- phase 1 computed them once and only if phase 1 itself geocoded, so a phase 2 correction left them empty (14 records) or routed to phase 1's wrong destination (5 records, worst 1.32 km off). The pipeline fix is `payload_builder.refresh_routing_metrics`; this repairs the history. Runs on the kiosk, needs OSRM and `XDG_RUNTIME_DIR`. `--apply` to write. |
