...
| `compile_mbtiles.py` | Crawls the City's own `Imagery_2025` tile cache into `ortho.mbtiles`: about 6 hours at ~20 requests a second. It skips tiles the archive already holds, so a rebuild goes to a staging directory (`--tiles-dir`) and is swapped in, then `finalize_mbtiles.py`. Which imagery the displays show today: the `mbtiles-tile-server` skill, §1. |
