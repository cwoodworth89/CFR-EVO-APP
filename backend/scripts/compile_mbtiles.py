...
- Optional per-layer rate limiting for municipal servers (see RateLimiter)
...
def compile_layer(
    ...
    rate = config.get("rate_limit_sec", 0.0)
    ...
    if limiter:
        logger.info(f" Rate Limit   : {rate:.3f}s between requests (~{1/rate:.0f} req/s CEILING,"
                    f" not multiplied by the {workers} workers)")
    logger.info("=" * 70)
    ...
    if not tiles_to_download:
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            future_to_tile = {
                executor.submit(download_tile, t, config, 3, limiter): t
