...
| Type size on the operator's laptop | *"We got some major work to do on text sizing for my dev laptop."* (screenshot: a 26-character intersection address at 72 px wrapped, the header half of a 1000-tall screen) | Header type from `lg` up now scales with viewport height, the canvas's 1080 sizes as the ceiling (address 6.5 vh, incident 2.8, first-due ETA 3.4, elapsed 3.8), and a long address shrinks by length to stay on one line. Rendered on DISP-2026-ABB77B (Lansdowne Dr & Aberdeen Ave): one line at 1916×1000, header about a fifth of the screen; two lines at 1280×800. The ergonomics skill records the rule. |
| The hydrant card's place and size | *"The hydrant data is in a bad spot. It's covering the route"*, *"it's way too big"*, then *"moving it to the units card. Or possibly to the timer card next to it. Review replay, exit review can be located somewhere else as they aren't on dispatches. We can put elapsed time maybe out of the way in the main header card, bottom right? Let's also shrink the hydrant/units/etas in text size"* | **The header is now the call, the units and the hydrant.** The hydrant took the card the clock vacated rather than a row inside the units card: the units card grows downward with the unit count and would have moved the hydrant about the screen, against the canvas's own "always in the same place at 03:00". Elapsed, the auto-dismiss state and the dismiss button are small at the foot of the call card, right-hand side; REVIEW REPLAY / EXIT REVIEW are a full-width strip above the header, beside the queued-call and Tier 1 banners, and render on no real dispatch. Units, ETAs and hydrant type stepped down about a fifth. Nothing floats over the map's lower half any more, so the destination cannot be covered. TAP TO ZOOM still drives the map's SNAP TO CALL, as a request counter (writing a handle into a prop ref is what the crash lint's `react-hooks/immutability` rule forbids). |
| Where the amber banners go, and the hydrant card's alignment | *"move the hydrant card to the top align"*, *"make sure amber banners stack up there as well, ideally under the purple banner if it's shown"*, *"there is also a floating/pulsing amber banner that's been popping up in the main map board. I think we can remove that since we're using the above header banners"*, and a documents slot *"under hydrants (bottom aligned on the card) for bringing up a stored pre-incident plan document, or other special documents"* | **Nothing floats over the route map now** but the route pill and the control stack, both in its top corners. The three amber cards that did -- unresolved location, approximate location, street section -- are banners above the header, stacked under the purple review strip; the unresolved one was a duplicate of the banner already there, the other two were not, so they moved rather than went. The hydrant reads from the top of its card and the documents sit at its foot: the pre-incident plan when the record carries one (it opens the existing PrePlanModal, and left the notices row, which is now the arrival-point ruling alone), else *NO STORED DOCUMENTS*, since a control that opens nothing is worse than a stated absence (CLAUDE.md §6.1). "Other special documents" is not a feature; the slot is marked as the placeholder it is, and what those documents are is a post-freeze question. The smoke script learned that an unresolved call is a state, not a failure: it now asserts the awaited marks and the absence of SNAP TO CALL instead of demanding figures. |
| Setting an arrival point from a replay | *"I can set the streetview but I can't easily set a new arrival point without backing out and typing in the address"*, then on the first attempt *"I don't like this arrival point solution. It squishes the other PIP screens ... a button in review mode ... maybe we can actually put that button in the purple banner on the left of 'exit review'"* | **Built where the operator put it.** *Set arrival point* is in the purple review strip beside *Exit review*, and the card opens in the aerial tile's place; closing it gives the aerial back, so the dispatch display keeps its shape and the tiles their size. A click on the route map sets the point, with a dashed amber draft pin until it is saved. Shown on a replay only: the strip does not exist on a live call, so neither does the button. Gated on the replay rather than the padlock at the operator's word (reaching a replay means the review screen was unlocked); the API's `require_admin` on the save is unchanged. The console's own logic was lifted into `hooks/useArrivalPoint.js` and both surfaces now use it, rather than a second copy; the console's real parcel path was re-checked against the kiosk afterwards. The card says the ruling applies to the next call at that address, because the parcel row is the record for every future call there. |
| Dead space, and the tiles' black header bars | *"Can we eliminate some of the dead space. Under call type, in the hydrant card (get rid of the first choice is 500ft away text, that's clear). Top align the units. Can we do something about the black headers inside the pip cards ... something transparent maybe? The auto-dismiss paused can also go ... it's already noted in the purple banner."* | The header went from about 360 px to 245 at 1916×1000. The call card's foot no longer pushes to the bottom; the units read from the top as the hydrant does; the LONG LAY chip carries its own message and the sentence under it is gone; the tile headers are a gradient over the picture instead of a bar above it, so the tile is all picture, and they sit at the **right** because Google puts its own *Open in Maps* control at a Street View's top left. *Auto-dismiss paused* is gone from the call card on a replay, where the purple strip says it; on a live call the line stays, because there it is the only place that says whether the call clears itself. |
| A refused arrival-point save looked like a lost one | *"When I click save arrival point, the new marker disappears and I don't think it is setting"* | **Reproduced 2026-09-10** by driving the built UI against the kiosk API: placing works and the draft pin draws, and the save is **refused, not lost** — `POST /api/parcels/entrance` answers `401 Admin unlock required for this change.` and writes nothing (the parcel row for 2929 Barnet Hwy still had a null `entrance_lat` afterwards). The failure was one 10 px red line under the form; closing the card then cancelled the placement and took the pin with it, which is what read as "it disappeared". The error is now a red block saying **NOT SAVED**, and for this case it names the padlock and says the pin is still there. The 401 itself is correct: `require_admin` gates the save, and the review screen's unlock is a separate client-side session that can outlive the token. |
| Dead space in the header | *"Leave the address line alone"*, *"remove the EMERGENCY / ROUTINE badge — the screen's coloured outline already carries response mode"*, *"group Map Grid + talk group + elapsed time together"* | **Measured first, and the measurement moved the plan.** The foot row stood 50 px because its right-hand stack (Elapsed label, clock, auto-dismiss line) did, while the talk-group chip beside it was 35; the response badge cost **nothing**, since it shared the incident row. So the badge was not the saving and deleting it would have left colour as the only channel for the distinction — red against green, with the word gone. It is a word in the new status line instead: no badge, no pulse, no height. The saving came from the line itself and from giving the address its own row: `GRID · TG · response · ELAPSED` at 22 px, and a long intersection address that no longer wraps. Header at 1916×1000: 225→197, 196→182, 196→168. At 1280×800: 221→193, 196→182, and **240→168** for the long address. Every pixel went to the map and the tiles. This supersedes the 2026-09-10 ruling that put the clock at bottom right. |
