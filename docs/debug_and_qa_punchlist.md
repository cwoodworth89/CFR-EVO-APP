...
|:--|:--|:--|:--|
| **94** | 🟠 operational | BUILT `ce7d4417` + `028f47a4`, in the bundle lead built 2026-09-16 23:24 local; **operator to repeat his steps**. A saved point moves the call on screen (pin, route, Street View aim, and now the header's unit ETAs, live from OSRM) and stays on close. Not a regression — a ruling change reversing `350c16d6`'s bound. Nothing open | [A saved arrival point is dropped from the call on screen when the editor closes](punchlist/94-a-saved-arrival-point-is-dropped-from-the-call-on-screen-when-the-editor-closes.md) |
