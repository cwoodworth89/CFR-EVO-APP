...
| **96** | 🔴 crew-visible | APPROVED 2026-09-19, with GIS. 584 of 1,994 junctions (29%) sit on a zone line but a candidate carries one grid, so the announced grid falsely flags a single junction and can fail to narrow a double crossing (selector). Fix: every zone within 5 m, filter by touch, rank by containment; backtest must lose no address | [A junction on a zone line carries one map grid, so the announced grid filters it out or flags it](punchlist/96-a-junction-on-a-zone-line-carries-one-map-grid-so-the-announced-grid-filters-it-out-or-flags-it.md) |
