...
|:--|:--|:--|:--|
| **92** | 🔴 crew-visible | DEPLOYED `eb90bcc7` 23:31Z **and the sync regressed: `ST_DWithin(::geography)` measured at ~3.7 s per record over ~6,500, the sync no longer completes.** Seven not yet confirmed served. Regression with gis-spatial-engineer now; the old code took ~1 s per sync | [Closures the City files on its own boundary roads are dropped by the spatial tests](punchlist/92-closures-on-boundary-roads-are-dropped-by-the-spatial-tests.md) |
