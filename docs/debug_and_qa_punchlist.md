...
|:--|:--|:--|:--|
| **92** | 🔴 crew-visible | REGRESSION FIXED `d112cf37` + both feeds ask for Coquitlam (DriveBC `bbox` from the table, Municipal 511 by publisher); sync measured 0.82 s on the kiosk's layers; **not deployed — one `--build api`, no migration.** MOTI copies dropped on the operator's word. Seven not yet confirmed served | [Closures the City files on its own boundary roads are dropped by the spatial tests](punchlist/92-closures-on-boundary-roads-are-dropped-by-the-spatial-tests.md) |
