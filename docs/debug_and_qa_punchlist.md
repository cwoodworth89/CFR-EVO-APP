...
|:--|:--|:--|:--|
| **91** | 🔴 crew-visible | BACKEND BUILT `ea6151b9` + migration (**apply before `--build api`**), frontend half with frontend-kiosk-architect; **not deployed**; text placeholders and three follow-on rulings still the operator's | [Road closure fields the feed did not send are filled with invented values, including the access level](punchlist/91-road-closure-fields-the-feed-did-not-send-are-filled-with-invented-values.md) |
| **90** | 🔴 crew-visible | BUILT `e3009d6a` + `2978da7e` + `73cd4e97`, **not deployed** — needs `--build api` and a frontend build; latent, 177 rows, 0 null on the kiosk | [A road closure with no coordinates is drawn at a hardcoded point near Coquitlam Centre](punchlist/90-a-road-closure-with-no-coordinates-is-drawn-at-a-hardcoded-point.md) |
