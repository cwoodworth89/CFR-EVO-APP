...
|:--|:--|:--|:--|
| **91** | 🔴 crew-visible | REWORK in progress — operator reversed the id ruling 2026-09-16 (skip, never serve) and ruled text placeholders (`--`) and the Municipal 511 default (null); `ea6151b9` + `776b52e5` **must not be deployed as they stand**; DriveBC pass-through pending a permitted spec read | [Road closure fields the feed did not send are filled with invented values, including the access level](punchlist/91-road-closure-fields-the-feed-did-not-send-are-filled-with-invented-values.md) |
