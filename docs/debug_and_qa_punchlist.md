...
|:--|:--|:--|:--|
| **93** | 🟠 operational | BUILT `46c0774d`, **needs a frontend build only**; metadata requests verified free (Google: "available at no charge", SKU unlimited); heading-0 root cause gone, every fallback branch tested, all 41 measured dispatches now get a heading. **Falsifier is the operator's replay of the 41** (41 free metadata + 41 billed-in-free-tier image requests, his permission) | [The Street View tile faces due north on every call with no saved view](punchlist/93-the-street-view-tile-faces-due-north-on-every-unsaved-call.md) |
| **92** | 🔴 crew-visible | **CONFIRMED** — deployed 23:48Z; every boundary record measured is served or correctly ended, Westwood left the feed (measured on the kiosk), RIDE-100086 served from DriveBC, MOTI 0; wall time is 83% Municipal 511 network, not ours. `2d179265` deployed 2026-09-17 02:58Z and confirmed: RIDE-100086 reads "Highway 7B". Parallel file fetch approved "if it's easy", with GIS | [Closures the City files on its own boundary roads are dropped by the spatial tests](punchlist/92-closures-on-boundary-roads-are-dropped-by-the-spatial-tests.md) |
