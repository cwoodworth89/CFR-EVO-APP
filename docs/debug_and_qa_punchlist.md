...
|:--|:--|:--|:--|
| **93** | 🟠 operational | RULED 2026-09-16 — option A (metadata request per call, aim at the lot centre): "Let's get A going because it's a crew facing issue." With frontend-kiosk-architect; billing to be verified before building | [The Street View tile faces due north on every call with no saved view](punchlist/93-the-street-view-tile-faces-due-north-on-every-unsaved-call.md) |
| **92** | 🔴 crew-visible | DEPLOYED `d112cf37` 2026-09-16 23:48Z, **confirmed in part**: sync completes (10.7 s wall), RIDE-100086 served from DriveBC in zone 52, four of the six City records served, one ended 09-12 (correct), **one — Westwood at Gordon — not ingested, cause with GIS**; MOTI 0. Also with GIS: DriveBC `street_name` null, and 1 s → 10.7 s wall | [Closures the City files on its own boundary roads are dropped by the spatial tests](punchlist/92-closures-on-boundary-roads-are-dropped-by-the-spatial-tests.md) |
