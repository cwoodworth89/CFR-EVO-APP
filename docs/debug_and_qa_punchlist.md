...
|:--|:--|:--|:--|
| **92** | 🔴 crew-visible | BUILT `eb90bcc7` (gis-spatial-engineer), **not deployed — needs `--build api`, no migration.** All seven measured within 7.8 m of the boundary and 27.3 m of a zone; fixed behind the operator's existing 100 m figure. DriveBC: only 2 events near the city today, the one inside was dropped and is now kept — not the systemic drop lead suspected | [Closures the City files on its own boundary roads are dropped by the spatial tests](punchlist/92-closures-on-boundary-roads-are-dropped-by-the-spatial-tests.md) |
