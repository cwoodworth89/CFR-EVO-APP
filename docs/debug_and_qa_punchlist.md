...
|:--|:--|:--|:--|
| **91** | 🔴 crew-visible | BUILT `ea6151b9` + `776b52e5` with a migration (**apply before `--build api`**); **not deployed**; text placeholders and three follow-on rulings still the operator's | [Road closure fields the feed did not send are filled with invented values, including the access level](punchlist/91-road-closure-fields-the-feed-did-not-send-are-filled-with-invented-values.md) |
