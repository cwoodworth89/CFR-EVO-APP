...
|:--|:--|:--|:--|
| **94** | 🟠 operational | OPEN — reported 2026-09-17 with a screenshot; a regression per the operator ("it used to do that"); the editor is review-mode only, so it is the operator's view that reverts, and the next live call takes the saved entrance from the parcel. With frontend-kiosk-architect to find the commit, then restore | [A saved arrival point is dropped from the call on screen when the editor closes](punchlist/94-a-saved-arrival-point-is-dropped-from-the-call-on-screen-when-the-editor-closes.md) |
