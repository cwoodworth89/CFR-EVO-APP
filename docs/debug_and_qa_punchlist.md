...
|:--|:--|:--|:--|
| **91** | 🔴 crew-visible | BACKEND REWORKED `c8dc474d` (supersedes `ea6151b9`), frontend rework with frontend-kiosk-architect; **not deployed** — migration (two `DROP NOT NULL`s, lead-checked) runs **before** `--build api`; admin surface and log volume are the operator's; DriveBC pass-through pending a permitted spec read | [Road closure fields the feed did not send are filled with invented values, including the access level](punchlist/91-road-closure-fields-the-feed-did-not-send-are-filled-with-invented-values.md) |
| **90** | 🔴 crew-visible | DEPLOYED `e3009d6a` + `2978da7e` (confirmed on the kiosk 2026-09-16: HEAD `fdfb103e`, built 15:18Z); placement follow-up `73cd4e97` **not deployed**; latent, 177 rows, 0 null | [A road closure with no coordinates is drawn at a hardcoded point near Coquitlam Centre](punchlist/90-a-road-closure-with-no-coordinates-is-drawn-at-a-hardcoded-point.md) |
| **89** | 🔴 crew-visible | DEPLOYED 2026-09-16 (confirmed on the kiosk: HEAD `fdfb103e`, built 15:18Z) **with a hook-order defect in `727c297b` that is live — fixed in `776b52e5`, needs a frontend build**; falsifier not yet run; "any for now" as built | [A failed road closure sync leaves the last list on screen, with no warning](punchlist/89-a-failed-road-closure-sync-leaves-the-last-list-on-screen-with-no-warning.md) |
