...
|:--|:--|:--|:--|
| **94** | 🟠 operational | BUILT `ce7d4417`, in the bundle lead built 2026-09-16 23:18 local; **operator to repeat his steps**. A saved point now moves the call on screen (pin, route, Street View aim) and stays on close; wording says it applies now and to future calls. **Not a regression:** the replay never moved the pin (`350c16d6`, 09-14, bounded deliberately); the console's Explore editor has done it since 09-06 and is what the operator remembers. Recorded as a ruling change. **Open for the operator:** header ETAs after a move — as recorded, or `--` | [A saved arrival point is dropped from the call on screen when the editor closes](punchlist/94-a-saved-arrival-point-is-dropped-from-the-call-on-screen-when-the-editor-closes.md) |
