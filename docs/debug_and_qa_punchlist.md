...
| **90** | 🔴 crew-visible | BUILT `e3009d6a` + `2978da7e`, **not deployed** — needs `--build api` and a frontend build; latent, 177 rows, 0 null on the kiosk | [A road closure with no coordinates is drawn at a hardcoded point near Coquitlam Centre](punchlist/90-a-road-closure-with-no-coordinates-is-drawn-at-a-hardcoded-point.md) |
