...
**23 open** (17 crew-visible) · **83 closed** → [`punchlist/_closed.md`](punchlist/_closed.md)
...
| **95** | 🔴 crew-visible | **FIXED, RUNNING, and the call re-recorded** (`f91bdedd`; agent restarted 16:44 PDT; `tools/reparse_dispatch.py` `75dfe724` rewrote row 822 with the junction and an E4 route, three older calls left as captured on the operator's word). Live call `A018E9` showed LOCATION UNRESOLVED: the address loop used canonical call-type spellings only, so "smoldering" stayed at the head of the intersection. Backtest 639 calls: 3 addresses gained, 0 lost | [The words the call type was read from are left at the head of an intersection address](punchlist/95-the-words-the-call-type-was-read-from-are-left-at-the-head-of-an-intersection-address.md) |
| **96** | 🔴 crew-visible | BUILT `92ed4aa0`, **needs a `cfr-agent` restart** (held for the bundle). Candidates carry every zone within 5 m; filter by touch, rank by containment. Measured over all 80 double crossings: 18 outcomes change, all from the false note, 0 regressions; geocode harness 642 calls, 6 gains, 0 losses. `GRID_MISMATCH` on the same junctions backlogged | [A junction on a zone line carries one map grid, so the announced grid filters it out or flags it](punchlist/96-a-junction-on-a-zone-line-carries-one-map-grid-so-the-announced-grid-filters-it-out-or-flags-it.md) |
...
| **91** | 🔴 crew-visible | Second round **DEPLOYED 2026-09-16 20:59Z, confirmed** (api image 20:59:49Z, kiosk HEAD `17a2c878`, sync 21:00Z `SUCCEEDED`, contract keys on all 71, log 141 ERROR → 2 WARNING lines). Road-state mapping **unexercised live: zero DriveBC rows have ever entered the table** — that is #92. **Third round DEPLOYED 2026-09-17 17:11Z, confirmed by tally:** api `e2a7f565` (tiers + text elevates), frontend `5b79a3f7` (four buckets, Info and Unspecified hidden by default). One serve: 52 CAUTION / 11 INFO / 9 N/A / 3 ACCESS_ONLY / 2 NO_ACCESS against the code's 52 / 10 / 10 / 3 / 1 — the difference is feed churn. Local Traffic Only and Detour → Full Closure ruled "for now", `37919d9e`, **deployed and tallied 2026-09-18 23:32Z** (Casey St reads FULL CLOSURE). Nothing open on this item | [Road closure fields the feed did not send are filled with invented values, including the access level](punchlist/91-road-closure-fields-the-feed-did-not-send-are-filled-with-invented-values.md) |
