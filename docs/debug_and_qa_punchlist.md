...
|:--|:--|:--|:--|
| **92** | 🔴 crew-visible | OPEN — found 2026-09-16 by the operator in the #91 review list; **and the table shows zero DriveBC rows ever, so the same tests likely drop every highway event** — with gis-spatial-engineer to measure, then fix | [Closures the City files on its own boundary roads are dropped by the spatial tests](punchlist/92-closures-on-boundary-roads-are-dropped-by-the-spatial-tests.md) |
