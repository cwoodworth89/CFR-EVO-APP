...
| **95** | 🔴 crew-visible | FIXED `f91bdedd`, pushed; **needs kiosk `git pull` + `cfr-agent` restart (operator), no api rebuild.** Live call `A018E9` showed LOCATION UNRESOLVED: the address loop used canonical call-type spellings only, so "smoldering" stayed at the head of the intersection. Backtest 639 calls: 3 addresses gained, 0 lost | [The words the call type was read from are left at the head of an intersection address](punchlist/95-the-words-the-call-type-was-read-from-are-left-at-the-head-of-an-intersection-address.md) |
