...
|:--|:--|:--|:--|
| **91** | 🔴 crew-visible | DEPLOYED 2026-09-16 20:07Z (confirmed on the kiosk, HEAD `0d17989a`); first sync forced 20:08Z: **67 of 71 active closures are N/A**, 141 ERROR lines in 3 min — log volume is the operator's to rule; admin panel row BUILT `e0514fe4`, **needs a frontend build**; DriveBC severity vocabulary verified, pass-through question open pending `roads[].state` | [Road closure fields the feed did not send are filled with invented values, including the access level](punchlist/91-road-closure-fields-the-feed-did-not-send-are-filled-with-invented-values.md) |
...
| **89** | 🔴 crew-visible | DEPLOYED, hook defect cleared by the 20:07Z build (confirmed on the kiosk, HEAD `0d17989a`); forced sync 20:08Z `SUCCEEDED`, both feeds reached; **falsifier (link pulled for one tick) not yet run**; "any for now" as built | [A failed road closure sync leaves the last list on screen, with no warning](punchlist/89-a-failed-road-closure-sync-leaves-the-last-list-on-screen-with-no-warning.md) |
