...
|:--|:--|:--|:--|
| **93** | 🟠 operational | DEPLOYED and **seen live on 2985 Delehaye: the tile aims correctly**, so the key admits the metadata endpoint. **Regression found by the operator: Expand snaps back on every arrow, pan or zoom** — with frontend-kiosk-architect now, urgent. Replay of the 41 still the falsifier | [The Street View tile faces due north on every call with no saved view](punchlist/93-the-street-view-tile-faces-due-north-on-every-unsaved-call.md) |
