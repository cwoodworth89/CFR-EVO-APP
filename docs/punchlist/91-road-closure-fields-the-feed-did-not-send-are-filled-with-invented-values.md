...
| | |
|:--|:--|
| **Status** | First round DEPLOYED 20:07Z. **Second round built, not deployed:** `a002ce82` (DriveBC access from `roads[].state`, needs migration `2026-09-16c` **before** the rebuild), `2f767903` (one WARNING line per serve), `e0514fe4` (admin panel row), `bf8b7e2a` (Municipal 511 value table, measured, and the operator's review list). Frontend render of the new contract in progress. **Operator to rule:** the municipal tiers and filters from the review list; GIS's three domain choices (most-restrictive road; `CLOSED` + bad direction → NO_ACCESS; Info as null + `roadState`) |
...
| 2026-09-16 | **Pieces A, B and C built.** **A — `a002ce82`:** `_drivebc_access` replaces `_drivebc_severity` (`road_closure_service.py`; constants `OPEN511_ROAD_STATES` / `_DIRECTIONS` / `_SEVERITIES` cited to the spec); new columns `road_state`, `road_direction`, `feed_severity` (`models.py:104–106`), served as `roadState`, `roadDirection`, `feedSeverity` — all null for Municipal 511. Tiers as ruled: `CLOSED` + BOTH/NONE/absent → NO_ACCESS; `CLOSED` in one direction → CAUTION with the direction carried; SOME_LANES_CLOSED / SINGLE_LANE_ALTERNATING → CAUTION; ALL_LANES_OPEN → `emergencyAccess` null with `roadState` stating it; absent → null; out-of-spec state → null + one ERROR. `feedSeverity` never sets a tier. **Three choices GIS made and flagged as its own, not ruled:** (1) **Info is `emergencyAccess` null + `roadState` ALL_LANES_OPEN**, not a fourth enum value — the deployed filters would show an unknown value unfiltered; the frontend distinguishes Info from N/A in the helper; (2) **several roads on one event: the most restrictive wins** (NO_ACCESS > CAUTION > ALL_LANES_OPEN > unknown, ties to feed order, WARNING when they disagree) — a closure on any listed road is never hidden behind an open one; (3) **`CLOSED` with an out-of-spec direction → NO_ACCESS**, treated as an absent direction, because the road is stated closed and an unknown direction should not turn it into N/A. Migration `2026-09-16c_road_closure_open511_road_state.sql`: three nullable `ADD COLUMN IF NOT EXISTS`, one transaction — **lead read it, matches the model, safe under old code; required before the rebuild** (`test_road_closures_router` against the kiosk fails with `UndefinedColumn road_state` until it runs). 30 road-closure tests + 25 subtests on the measured shapes, all eight directions, both unused spec states, the multi-road case; nothing live. **B — `bf8b7e2a`:** the `RoadClosureType` value → label table **measured from the vendor's `Framework.min.js`** (every line measured, none inferred; the vendor and our code both read the highest bit) — 20 values, in the standards row now ✅ HELD for the mapping, still no licence. The pull was **16 requests, not the 2–4 estimated** — the Coquitlam page lists 13 data files — registered with the correction; the files hold every Transnomis client (~6,500 issues), the kiosk's own two spatial tests cut it to Coquitlam. **Review list for the operator:** `docs/briefings/municipal511_coquitlam_records_2026-09-16.md` — 71 current records, **exactly the kiosk's 71 served** (the cross-check), grouped by label with tier as served, plus 7 past end date and 13 dropped by the spatial tests; nothing filtered, mapping unchanged. Surfaced there: filter candidates Sidewalk (1), No / Minimal Traffic Impact (9), Unknown (10); **32768 "Emergency Access Unspecified" — the vendor's legend groups it with No Emergency Access, its overlay code draws it with the *with*-access icon**, we serve ACCESS_ONLY; Lane(s) Closed (46), Alternating, One Direction and Intermittently Blocked are **null while their DriveBC counterparts are now CAUTION**; 16384 "Local Traffic Only" is served ACCESS_ONLY, which is not the same thing as emergency access; the text rule raises one Alternating record ("full closure dec 5"); **six records the City filed itself are dropped by the spatial tests** (North Rd ×3, Westwood St, Victoria Dr, Balmoral Dr) plus a Mary Hill Bypass MOTI record — why each falls outside was not checked. **C — `2f767903`:** the per-record serve-time line is one **WARNING** per uncached serve with the count by source; the id-less skip and #90's coordinate stay ERROR per record. Frontend render of the A contract sent to `frontend-kiosk-architect` |
