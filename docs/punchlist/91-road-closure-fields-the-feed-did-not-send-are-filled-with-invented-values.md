...
| | |
|:--|:--|
| **Status** | BUILT — backend `c8dc474d`, frontend `bbfc393e`, superseding `ea6151b9` + `776b52e5`. **Not deployed, migration not run, nothing seen rendered.** Order: migration, then frontend build, then `--build api`. Admin surface (proposed, not built) and ERROR log volume are the operator's. DriveBC present-severity pass-through pending a permitted spec read |
