...
| | |
|:--|:--|
| **Status** | REGRESSION FIXED and both feeds ask for Coquitlam — `d112cf37` (code, tests, §2.1) + `33c36ee6` (register). **Not deployed: one `--build api`, no migration.** Whole sync measured 0.82 s on the kiosk's layers against 1.02 s before #92 and "stopped at 257 of 9,596 after 161 s" for `eb90bcc7`. Seven not yet confirmed served; falsifiers below |
