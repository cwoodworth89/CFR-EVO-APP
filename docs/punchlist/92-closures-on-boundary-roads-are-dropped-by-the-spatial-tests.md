...
| | |
|:--|:--|
| **Status** | DEPLOYED `eb90bcc7` 2026-09-16 23:31Z (api image 23:31:09Z, kiosk HEAD `fa8cfd4e`) — **and the sync regressed to hours: not confirmed, seven not yet served.** `is_within_city`'s `ST_DWithin(cb.geom::geography, …)` measured at ~3.7 s per record on the kiosk, run once per record over the full ~6,500-issue pull. With GIS to fix and re-measure; one more rebuild after |
