...
| | |
|:--|:--|
| **Status** | DEPLOYED — `46c0774d` and the amendment `dade8273` are in the bundle lead built on the kiosk 2026-09-16 20:02 local (state check SAFE, no restart). **Nothing seen rendered.** Billing verified free before building. The operator's replay of the 41 dispatches is the falsifier; the first live call also confirms the kiosk key's restriction admits the metadata endpoint (unverified, medium) |
