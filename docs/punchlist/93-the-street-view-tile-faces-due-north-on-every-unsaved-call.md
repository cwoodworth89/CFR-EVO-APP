...
| | |
|:--|:--|
| **Status** | DEPLOYED, and **seen live by the operator on 2985 Delehaye: the compact tile points the right way** — which confirms the kiosk key admits the metadata endpoint. **Regression, crew-visible in Expand: every arrow travel, pan or zoom snaps back to the original spot.** With `frontend-kiosk-architect`, urgent. The replay of the 41 remains the falsifier for the aim |
