...
| | |
|:--|:--|
| **Status** | BUILT `46c0774d` (five files, `-o`), **needs a frontend build only — not deployed, nothing seen rendered.** Billing verified free before building. The operator's replay of the 41 dispatches is the falsifier; the first live call also confirms the kiosk key's restrictions allow the metadata endpoint (unverified, medium) |
