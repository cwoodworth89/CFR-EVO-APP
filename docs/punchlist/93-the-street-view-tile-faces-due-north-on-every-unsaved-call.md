...
| 2026-09-17 | **Second live confirmation, operator, on the Heffley replay:** setting an arrival point in review "automatically snaps the unset streetview to the new area, and rotates it to face the property. Super handy. Document that's welcome behaviour." That is #94's moved target arriving as a new call key — one metadata request, re-aimed at the lot centre from the nearest panorama to the new point — the once-per-key rule's one intended re-aim, seen working. Recorded as intended in the `google-imagery-streetview` runbook by the frontend agent |
