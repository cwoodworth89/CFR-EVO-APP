...
| | |
|:--|:--|
| **Status** | OPEN — reported 2026-09-17 by the operator with a screenshot; with `frontend-kiosk-architect` to trace, then fix |
| **Severity** | 🔴 crew-visible — on a live call the operator sets the entrance, the route draws to it, and the moment the editor closes the crew's route snaps back to the frontage with nothing saying so |
