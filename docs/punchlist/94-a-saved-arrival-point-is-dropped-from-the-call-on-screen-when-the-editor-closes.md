...
| | |
|:--|:--|
| **Status** | BUILT `ce7d4417`, in the bundle lead built on the kiosk 2026-09-16 23:18 local (state check SAFE). **Nothing seen rendered — the operator repeats his steps.** **Not a regression — a ruling change** (see the log): the replay never moved the pin; the console's Explore editor has since 09-06. Header ETAs after a move **ruled**: they follow the moved destination, live from OSRM ("informational and not statistical"); with the frontend agent |
