...
| | |
|:--|:--|
| **Status** | BUILT `ce7d4417` (four files, `-o`), in the bundle lead built on the kiosk 2026-09-16 23:18 local (state check SAFE). **Nothing seen rendered — the operator repeats his steps.** One question open for him: the header's recorded ETAs after a move. Whether the old behaviour was ever in the code (the operator believes so; the code comment documented the revert as design) is still to come from the agent |
