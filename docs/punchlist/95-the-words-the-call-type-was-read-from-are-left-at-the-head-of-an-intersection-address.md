...
| | |
|:--|:--|
| **Status** | FIXED `f91bdedd`, pushed, **not yet running: needs `git pull` on the kiosk and a `cfr-agent` restart (operator)**. Reproduced, geocode of the fixed string confirmed against the kiosk DB, backtest over 639 verified calls: 3 addresses gained, 0 lost |
