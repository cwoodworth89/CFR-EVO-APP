...
| **`is_test` reaches the dispatch row** (backlog, operator-approved 2026-09-19): folded into `target` at the API boundary on create and update; absent stores nothing. **`--build api` pending**, no agent restart; trustworthy from the deploy date on | `3096dbc6` |
