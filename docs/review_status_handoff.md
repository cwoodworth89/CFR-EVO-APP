...
| **Admin dashboard loads in pages** (operator, 2026-09-18: "find the worst offenders"): `GET /api/dispatches?summary=true` and `/api/evaluations?summary=true` (list columns only; the full record on selecting a row), evaluations off the critical path, newest 100 first with Show all, badge green when the first page lands. Measured before: 2.4 MB + 1.8 MB serial; after (projected from the live rows through the new serialisers): 92 kB first page, ~512 kB remainder in the background, 42 kB evaluations. Frontend built on the kiosk; **api rebuild pending** | `5943276d` |
| `65a50d15` |
