...
| Road closure severity vocabulary | DriveBC Open511 v1 — `api.open511.gov.bc.ca/help` and BC's OpenAPI specs (`github.com/bcgov/api-specs/…/open511/`, via the Government of Canada open data record the operator supplied, dataset `23a839e3-8fb4-4569-bb3d-c28a7621f687`), all read 2026-09-16 with the operator's permission; **the feed is Open Government Licence – British Columbia** (from the spec's `license`); Municipal 511's `RoadClosureType` | ✅ HELD for DriveBC `severity`, `event_type`, `status` (as read, not vendored); 📏 MEASURED for `roads[].state` (live feed, 2026-09-16); ⚠️ NOT HELD for the full `roads[].state` enumeration and for Municipal 511 | **DriveBC `severity` is MINOR / MODERATE / MAJOR / UNKNOWN, every definition about traffic impact** (MAJOR: "a significant impact on traffic, probably on a large scale"), **not passability** — so `road_closure_service.py`'s MAJOR → NO_ACCESS is unsourced and stays only until the operator rules on pass-through (punch-list #91, ruling 5). `event_type`: CONSTRUCTION / SPECIAL_EVENT / INCIDENT / WEATHER_CONDITION / ROAD_CONDITION. The passability field is `roads[].state` (DriveBC supports `name, from, to, state, direction` + custom `delay`). **No reachable document enumerates it**: DriveBC's help page shows only `"CLOSED"`; BC's OAS2 and OAS3 specs define query parameters and no response schema; `open511.org` answers with a self-signed certificate (the operator hit the same wall). **Measured instead, on the live feed 2026-09-16 (306 active events, one request):** `roads[].state` ∈ {`ALL_LANES_OPEN` 238, absent 52, `CLOSED` 16}; `severity` ∈ {MINOR 274, MAJOR 32} — no MODERATE or UNKNOWN in use; `event_type` ∈ {CONSTRUCTION 273, INCIDENT 18, ROAD_CONDITION 15}. **Severity and state are independent:** 18 MAJOR events had `ALL_LANES_OPEN`, 5 MINOR events were `CLOSED`. Any state value outside the measured set is unknown until the operator rules on it. Municipal 511: an unstated type is NULL → N/A since `c8dc474d`; what its type bits mean is the department's reading of the City feed, not a published standard. |
