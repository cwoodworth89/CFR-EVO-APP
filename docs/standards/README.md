...
| Which hydrants a driver is shown for a call | Department operational policy, operator ruling 2026-09-06 (300 ft approach along the route, then around the address, then the 1,000 ft supply hose carried) | ✅ HELD (ruling recorded in punch-list #74; constants cited in `frontend/src/utils/routeHydrants.js`) | The 30 m route band is measured on `public.hydrants` vs `public.roads` (p95 35 m), not policy. `NOT READY` excluded. |
