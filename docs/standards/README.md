...
| Response-time truth | E-Comm CAD, from apparatus GPS (operator, 2026-09-17: "Run time truths are stored in the CAD software using truck GPS") | ⚠️ NOT HELD here — held by CAD | **Every ETA, distance and travel time this system shows is informational, never statistical.** The record of what a run actually took is CAD's, from truck GPS, and nothing in this project reproduces or replaces it. So a displayed ETA follows the destination on screen (OSRM live, §6.2), and `routing_metrics` stored with a dispatch is what the kiosk *showed* at dispatch time — a fact about the display, not about the run. Anything for department leadership about response times cites CAD, not this system (see the `performance-metrics-analytics` runbook). Punch-list #94. |
...
| Building locations on multi-building sites (Riverview, 2601 Lougheed Hwy, and the like) | **None held.** The City's parcel data is the address authority and carries one lot (98.7 ha for Riverview) with no buildings; E-Comm CAD announces a building ("Number 47 / Station 47") the City has no record of | ⚠️ NOT HELD — open gap (§7.5), recorded 2026-09-19 | Operator, 2026-09-18/19: "The city doesn't break down all the buildings on the properties, but dispatch does" and "it's not a base site in the traditional sense." So neither of our two authorities has the answer: the City has the lot, the CAD has the number, and the point on the ground for that number lives only in the department's own knowledge (pre-plans, site walks). If it is built, its provenance is §6.3 tier 4 — department operational data, entered and owned by the department, keyed by exactly what the CAD announces — not derived from City geometry or OSM footprints, which would be guesses wearing a source. Parked with the Riverview line in `../post_freeze_backlog.md`. |
