...
| Apparatus routing profile | The OSRM `car.lua` shipped in the image that built the kiosk graph (`ghcr.io/project-osrm/osrm-backend`, v26.8.0) | ✅ HELD as text (vendored 2026-09-08, [`osrm/README.md`](osrm/README.md)); ⚠️ NOT HELD as a fire-apparatus authority | The live profile is stock `car.lua`, unmodified: the vendored copy is the text any change cites by line (CLAUDE.md §7.3a), and its README records the evidence that it built the graph. No document says how an engine or a ladder should weight roads; the operator is the authority (§6.3 tier 4). **Rulings 2026-09-09**, quoted in punch-list #1: one rule set for every apparatus, emergency response assumed; fire lanes (`service=emergency_access`) drivable; one-way never offered; bollards not offered; no weight or height limits apply. Posted turn restrictions: ignored, the turns are made under lights and siren (`use_turn_restrictions = false`). The rule set is fully specified and not yet applied. **Open**: the speed table, which is a measurement, not a ruling. |
