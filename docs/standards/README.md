...
| Road closure severity vocabulary | DriveBC Open511 v1 — `api.open511.gov.bc.ca/help` (the jurisdiction's own documentation, read 2026-09-16 with the operator's permission); Municipal 511's `RoadClosureType` | ✅ HELD for DriveBC `severity` and `event_type` (as read, not vendored); ⚠️ NOT HELD for `roads[].state` values and for Municipal 511 | **DriveBC `severity` is MINOR / MODERATE / MAJOR / UNKNOWN, every definition about traffic impact** (MAJOR: "a significant impact on traffic, probably on a large scale"), **not passability** — so `road_closure_service.py`'s MAJOR → NO_ACCESS is unsourced and stays only until the operator rules on pass-through (punch-list #91, ruling 5). `event_type`: CONSTRUCTION / SPECIAL_EVENT / INCIDENT / WEATHER_CONDITION / ROAD_CONDITION. The passability field is `roads[].state` (DriveBC supports `name, from, to, state, direction` + custom `delay`); its enumeration is not on DriveBC's page and the Open511 spec site (`open511.org`) answers with a self-signed certificate — not read. Municipal 511: an unstated type is NULL → N/A since `c8dc474d`; what its type bits mean is the department's reading of the City feed, not a published standard. |
