...
| Road closure severity vocabulary | Open511 (DriveBC's `open511.gov.bc.ca` event `severity`); Municipal 511's `RoadClosureType` | ⚠️ NOT HELD | `road_closure_service.py` maps DriveBC `MAJOR` → NO_ACCESS and every other present value → CAUTION by fall-through; that Open511 enumerates MINOR/MODERATE/MAJOR/UNKNOWN is recollection, unverified (§7.3). Municipal 511 starts at CAUTION and raises from type bits. An **absent** severity is NULL and shown as N/A since `ea6151b9` (punch-list #91, operator ruling 2026-09-16); what a *present* value means is the gap. Verify against the published Open511 spec before citing it. |
