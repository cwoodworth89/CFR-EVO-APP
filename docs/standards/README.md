...
| Which hydrants a driver is shown for a call | Department operational policy, operator ruling 2026-09-06 (a hydrant within 50 ft of the marker first, the 50 ft rolls carried; then along the route of travel within the 1,000 ft supply lay, the 808 Miller Ave ruling; then within 300 ft of the address; then the 1,000 ft supply hose straight-line) | ✅ HELD (ruling recorded in punch-list #74; constants cited in `frontend/src/utils/routeHydrants.js`) | The 30 m route band is measured on `public.hydrants` vs `public.roads` (p95 35 m), not policy. `NOT READY` excluded. |
