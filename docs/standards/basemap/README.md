...
because the kiosk has no WAN at 3 am (`../README.md`, *How to use this*).
...
| Positron style (ruled out 2026-09-09) | `positron-gl-style/style.json`, a light grey design | Code BSD 3-Clause, **design CC-BY 4.0** — [`positron-gl-style-LICENSE.md`](positron-gl-style-LICENSE.md) | The visual design is attributed to *MapTiler.com & OpenMapTiles contributors*; a derivative must say it was changed (CC-BY 4.0 §3(a)(1)(B)) | The corner credit names OpenMapTiles; the full notice is this file. **An in-app "Data licences" view is not built** (post-freeze) |
| OSM Bright style (the live map) | `osm-bright-gl-style/style.json`, which `tools/build_basemap_style.py` turns into the live style with the operator's 2026-09-09 rulings | Same terms — [`osm-bright-gl-style-LICENSE.md`](osm-bright-gl-style-LICENSE.md); derived from Mapbox Open Styles (BSD-3, 2014) | As above | As above |
...
| MapLibre GL JS | The renderer, `maplibre-gl` 4.7.1 in `frontend/package.json` | BSD 3-Clause — [`maplibre-gl-js-LICENSE-bsd-3.txt`](maplibre-gl-js-LICENSE-bsd-3.txt) | Keep the notice in the distribution | `node_modules`, and here |
| `@maplibre/maplibre-gl-leaflet` | The Leaflet plugin that hosts a MapLibre map under the existing Leaflet layers; 0.1.4, peers `maplibre-gl ^4.3.2` and `leaflet ^1.9.3` | ISC — [`maplibre-gl-leaflet-LICENSE-isc.txt`](maplibre-gl-leaflet-LICENSE-isc.txt) | Keep the notice | `frontend/package.json`, and here |
