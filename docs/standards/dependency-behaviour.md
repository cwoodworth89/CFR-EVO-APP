...
| `StreetViewPanorama` "ready" once a position is set | imagery is there to show | **it draws an empty, blank panorama when no imagery exists.** `StreetViewService.getPanorama` answers `google.maps.StreetViewStatus.ZERO_RESULTS` for that case; the name was taken from memory (the library loads from Google, no installed copy), then confirmed on screen by the operator 2026-09-15 at 6000 Quarry Rd, where the tile now reads "No Street View available" (`useStreetViewPanorama.js`, `9a788d94`) |
