...
export const BASE_LAYERS = {
  ...
  //
  // Since 2026-09-09: vector tiles the project builds itself from the OSM extract the kiosk
  // routes on (backend/scripts/build_vector_basemap.sh -> street_vector.mbtiles), drawn by
  // MapLibre GL under the Leaflet overlays with the project's own style
  // (frontend/public/basemap/street.style.json, generated by tools/build_basemap_style.py
  // from OSM Bright with the operator's rulings). It replaces the Carto raster crawl, whose
  // licence the project never held and whose tiles Carto watermarked (punch-list #47b); the
  // raster archives stay on the kiosk's disk as the rollback until the operator says otherwise.
  //
  // Labels are a style property here, not a second tile set: BaseMap shows or hides them,
  // and caps the road names at CADASTRAL_MIN_ZOOM while the cadastral overlay carries the
  // City's own. The old GREY/DARK/OSM/VOYAGER names and the `_nolabels` URL swap are gone.
  //
  // Licences and the credit line: docs/standards/basemap/README.md.
  STREET: {
    type: 'vector',
    // The archive's service name on cfr_tiles; the style, sprite and glyphs come from this
    // app's own origin (components/map/vectorBasemap.js).
    service: 'street_vector',
    attribution: '© OpenMapTiles · © OpenStreetMap contributors (ODbL)',
    // The tiles stop at z14 and MapLibre draws them at any zoom beyond: no crawled depth
    // to run out of, so no maxNativeZoom. 22 matches the other layers and the map.
    maxZoom: 22,
  },
