...
import ArrivalPointSection from './ArrivalPointSection';
          ...
          {nearest && (
            <div className="mt-3 pt-2.5 border-t border-slate-800/80 flex flex-col gap-1.5">
              ...
              <div className="flex justify-between text-xs bg-slate-950/80 px-3 py-1.5 rounded-lg border border-slate-800/80 font-mono">
                ...
                <span className="text-white font-black">{nearest.gisId} ({metresToFeet(nearest.distance) ?? '--'} ft{nearest.how === 'doorstep' ? ', within 100 ft' : nearest.how === 'approach' ? ' before arrival, on route' : nearest.how === 'supply' ? ', supply lay' : ''})</span>
              </div>
              {nearest.flowClass && (
                <div className="flex justify-between text-xs bg-slate-950/80 px-3 py-1.5 rounded-lg border border-slate-800/80 font-mono">
