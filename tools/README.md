...
| `route_corpus_baseline.py` | Routes every verified dispatch from all four hall aprons through the kiosk's OSRM, the arrival point resolved the way production resolves it, and keeps the answer: distance, duration, the road sequence, a geometry hash, nodes passed twice, U-turns. `--json` before a profile or graph change, `--baseline` after prints every route that moved by dispatch id and hall; `--record` (with `--label` naming the graph and profile) is the `routing` stage in `evaluation_history`. Built 2026-09-08 for the routing stream (`docs/briefs/osrm_routing_agent.md`). |
| `audit_skill_references.py` | Finds identifiers a `SKILL.md` names that exist nowhere in the code. `--scripts` checks this README. |
| `harness_phase1.py` | Phase-1 simulator: replays each recording in the listener's growing chunks through the real STT, parser and completion check, scores what the trigger would have published against the verified fields, and scores two candidate rules (a location gate, and agreement across two chunks). Kiosk only; `--limit 2` first. Punch list #72. |
...
| `osm_level_crossings.py` | Every `railway=level_crossing` node inside the City bounding box, read from the OSM extract OSRM was built from, each against the nearest hand-entered point in `railroadCrossings.js` and back. Kiosk only; needs `pip install osmium` in the venv. Punch list #21. |
